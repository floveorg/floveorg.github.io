...
### Shared CSS/JS · risa — doc vs code conflict (reported 2026-09-06)
- **SHC01** Reconcile the "Shared CSS and JS" plan (`development/index.html`, §"Shared CSS and JS") with the shipped reality in `risa/index.html`. Four discrepancies: (1) intended area `flove.org/appy/` vs actual `flove.org/central/shared/code/`; (2) intended inert `data-flove-css`/`data-flove-js` loader, additive & fail-silent, vs plain hard `<link>`/`<script src>` with no fallback; (3) doc still claims "hoy viven repetidos en la web" though `flove-feed/tags/player/bottom-nav/sound/app.js` + `flove.css/tabs/bottom-nav.css` are already extracted; (4) `appy` also names the app-catalogue app (`central/apps/appy/`), so routing shared code to flove.org/appy/ would collide. Existing `flove-loader.js` implements the documented markers but browsy-gift-gated (score ≥ `flove:gift-min`, default 40), not the doc's "siempre aditiva" — and risa doesn't use it. Decide: adopt a canonical shared path, wire the loader (or a fail-silent fallback) into the risa web, and update the doc's outdated claims → defer to Marc.
