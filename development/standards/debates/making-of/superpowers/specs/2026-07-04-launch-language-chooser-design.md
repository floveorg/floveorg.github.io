...
`localStorage['translaty-lang']` key to pick their language, defaulting to English
(the translaty engine — see `applyLang()`/`init()` in any translated app).
...
1. On load, read `localStorage['translaty-lang']`.
...
3. Click a button → `localStorage.setItem('translaty-lang', 'en'|'es')` →
...
double-click .desktop → start-flove.sh → serve root on :8642 → open launch.html
   launch.html: localStorage['translaty-lang']?
...
- **2 of 36 switcher-apps don't use `translaty-lang`** — they stay English regardless.
  Out of scope; noted for a later pass.

## Testing

- Fresh origin (clear `translaty-lang`): launch → chooser shows → pick Español →
...
- Additional languages (English + Spanish only, matching current translaty coverage).
