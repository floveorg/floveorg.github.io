...
- [ ] **Step 2 — Log the entry.** Add an appy-nety making-of entry: the driving prompts (basic→rich→advanced hub), the integration decisions (link-out vs embed; modes implemented all at once; ⚙ entry), and commit pointers `6ff9ff0 · d8194dc · 8d0efc0`. Per the questy-html convention, context = pointer-links to public Gitea commits, not embedded text.
