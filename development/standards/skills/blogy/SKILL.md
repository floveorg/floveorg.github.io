...
name: blogy
description: >-
  Write a flove development-log article for the blog (flove.org/development/blog, category
  "Tech") when Marc asks. Use whenever Marc types /blogy or asks to
  "write a blogy", "write up what's new", "do the monthly flove update", "blog
  the recent development", "milestone post", or otherwise wants a public write-up
  of what flove has been building lately. It gathers the real recent work (git
  log across the flove repos, memories, docs/making-of, standards changes),
  drafts ONE English article in the "Tech" category in flove's voice, and
  runs the drafts-first → preview → OK → publish flow. There is no automation:
  Marc triggers it (roughly monthly and/or at notable milestones). Built
  flove-first; it inherits the blog setup ([[project_flove_blog_blogy]]) and the
  publish pipeline ([[project_flove_publish_pipeline]]).
---
...
last blogy. Marc calls it when he wants one — not on a schedule.

## Editorial voice (Marc's standing preferences — honor them)
...
- **Spotlight standards.** Changes to the standards (the design contract,
  ...
  `flove.org/docs/standards.html`) deserve their own section; Marc cares about
  these especially.
...
4. **Preview + show Marc.** Start `hugo server -D --source ~/Documents/flove/development/blog`
   (background) and give Marc `http://localhost:1313/posts/<slug>/`. Summarize what
   the post says. **Do not publish yet** — this is drafts-first. Wait for his OK or
   edits; iterate.

5. **On Marc's OK:** flip `draft: false`, then publish:
...
- **Drafts-first, always.** Never publish before Marc has seen it.
- **One post per run.** Don't backfill several at once unless Marc asks.
- **Scoped commits.** Only the post file(s) you wrote; Marc edits in parallel.
