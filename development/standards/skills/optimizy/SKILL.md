...
description: >-
  ...
  (validity/a11y/mobile/i18n), export & share audit, a correctness code-review, and
  translation — in the one order that avoids rework and burns the fewest tokens. Use this
  ...
  for /simplify, /validaty, /exporty, /code-review and /translaty — reach for it even when
  the user only names two of those but clearly wants the app brought to a finished state. Built flove-first (single-file HTML/CSS/JS apps, the
  §13 standards, CSS-only navigation), and it inherits every standing rule of the skills it
  drives: inline surveys only, never a browser, no commit unless the user's workflow says so.
---
...
translated, reviewing bugs in code you're about to simplify away).
...
| 2 | **validaty** (`/validaty`) | Validity + a11y + mobile + leave i18n *ready*, on the structurally-final, single-language file. The `aria-label`s it adds get translated once, in stage 5. |
| 3 | **exporty** (`/exporty`) | Audit the export & share surface (§13.12) with the `summary-model` keys and labels already final. Any UI it adds (share button, format labels) lands before translation. |
| 4 | **code-review** (`/code-review`) | The **bug** pass, on clean, final, single-language code — smaller read, fewer false positives. The last correctness gate before the file inflates. |
| 5 | **translaty** (`/translaty`) | **Always last.** Wraps every now-final visible string (including whatever validaty/exporty added) + the worldball. Idempotent, so it sweeps the lot in one go. After this, nothing reads the file. |
...
Stages 1 and 5 are Anthropic built-ins. They're excellent at general code — and **blind to
...
| exporty | six format hooks + `navigator.share` + one `summary-model`, parse-back passes → skip (advanced/super only) |
...
For a single app, run the five stages in order, each as its own skill invocation, on the
same file. Between stages, keep a short running ledger of what changed — you'll fold it into
one final report rather than narrating five times.
...
- **Inherit, don't re-ask.** Each sub-skill has a standing rule to surface risky/divergent
  ...
  When running the full pipeline, **batch all the surveys you can foresee into one** at the
   ...
   interrupted five times. Where a sub-skill would block on a survey and you're running
...
The three flove sub-skills (`validaty`, `exporty`, `translaty`) each have an
...
| Tier / kind | simplify | validaty | exporty | code-review | translaty |
|-------------|:--:|:--:|:--:|:--:|:--:|
...
✓ applies · ~ light (little JS) · – near-empty, skip · N/A not applicable · opt optional.
...
its own gate** — validate can't reach `html-validate` 0, export parse-back fails, a
`node --check` errors — **halt that app before translaty** and report it. Translating an app
...
2. **Fan the per-app work out to subagents; keep only the conclusions.** When polishing a
   ...
   - Subagents **can't run `npx`/`node`** in the sandbox, so their validity and translaty
      ...
      **per app**, after the subagents return.
...
one that tracks app × {simplify, validaty, exporty, code-review, translaty}.
...
- For advanced/super: every export format **parses back** to the same item set as the summary
  (parity), and `navigator.share` is wired with the `canShare({files})` guard.
...
One report at the end, not five. For each app: which stages ran, which were skipped and why,
what was auto-fixed per stage, what was surveyed and how the user decided, the final
`html-validate` count and translaty gate result, and anything still open. Coverage honesty
over false completeness — a truthful "4 of 5 stages, validity at 3 errors remaining" beats a
...
- It does not invent stages. Five stages, that order. If the user wants only a subset
  ("just validaty + translaty"), run that subset *in this relative order* and say so.
