...
description: >-
  ...
  the vocabulary index (slot texts) into development/standards/vocabulary/index.html,
...
**defaults**; to fork this skill for another project, add
...
0. **Sync the vocabulary index (if your config enables it).** The package can
   carry the slot-vocabulary reference page (`development/standards/vocabulary/index.html`),
   ...
   - Refreshing the `vocab` object in
     `development/standards/vocabulary/index.html` so it mirrors the latest
     slot texts from the apps (titles, taglines, entry labels, about bodies).
   - Commit the refreshed page:
     `git -C {REPO} add development/standards/vocabulary/index.html` and commit
