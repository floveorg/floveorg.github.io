...
What it gives you and what it can't:

- **Gives:** every class/id/input-name/data-attr/i18n-key, all `sites` lines for each (the
  basis for an atomic rename), a `load_bearing` flag set only when a **state** selector
  (`:checked`, `:has(`, `:target`, `:focus-within`) or a `for=`/`:has(#id)`/`aria-*`
  reference keys on it — plain styling selectors are *not* flagged — the resolved `label`
  from `<label for=>`/`aria-label`, a `maybe_legacy` flag, the `data_objects` (deck→items),
  and `legacy_markers`.
- **Can't (do these by hand):** map a *semantic key* (`stats.main_triads`) or a `tKey`
  namespace to its label — labels for those live in i18n tables, not `for=`; judge whether a
