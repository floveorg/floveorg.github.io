...
description: >-
  ...
  `/exporty metas/souls.html`); audit only that file. Built flove-first (summary-model, rating
  Views, CSS-only nav). Anything that changes the UI or diverges from the shared flove standard is
...
orchestrator tells you so), audit the four axes and apply the Safe fixes in full — but keep
off the neighbouring stages' ground:
